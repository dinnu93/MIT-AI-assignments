...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        ...
        for i in expr1:
            for j in expr2:
                res.append(Product([i,j]))
        return Sum(res).simplify()
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        res = []
        for i in expr1:
            res.append(Product([i, expr2]))
        return Sum(res).simplify()
    elif isinstance(expr1, Product) and isinstance(expr2, Sum):
        res = []
        for j in expr2:
            res.append(Product([expr1, j]))
        return Sum(res).simplify()
    elif isinstance(expr1, Product) and isinstance(expr2, Product):
        if len(expr1) == 1 and expr1[0] == 1:
            return expr2
        elif len(expr2) == 1 and expr2[0] == 1:
            return expr1
        elif len(expr1) == 1 and len(expr2) == 1 and isinstance(expr1[0], int) and isinstance(expr2[0], int):
